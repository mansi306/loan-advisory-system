...
# Fix path for both local and cloud
root = Path(__file__).parent.parent
sys.path.insert(0, str(root))

# Train model if not exists — MUST happen before any src imports
...
# ── Main panel ─────────────────────────────────────────
if not submitted:
    st.info("Fill in the applicant details in the sidebar and "
            "click **Check Eligibility** to get a prediction.")
    ...
    with tab2:
        ...
        st.caption(
            ...
            "Negative = hurts approval. Positive = helps."
        )
    ...
    with tab3:
        if approved and prob >= 0.6:
            ...
            st.caption("Address the top item first.")
